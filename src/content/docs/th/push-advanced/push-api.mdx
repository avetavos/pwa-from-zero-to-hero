...
แอปที่ขอ permission แจ้งเตือนเฉพาะหลังจาก action ที่เกี่ยวข้อง (เช่นส่งข้อความ) แทนที่จะขอตั้งแต่แรกเข้า และแอปที่ส่ง reminder ตามเวลา (เช่น check-in reminder ที่ผูกกับตอนจองตั๋ว) ล้วนใช้หลัก "ขอ ณ จังหวะที่เกี่ยวข้อง" แบบเดียวกับที่บทเรียนนี้แนะนำ

export const quizPushApi = [
  {
    q: 'pushManager.subscribe() return อะไร?',
    options: [
      'ค่า boolean บอกว่าสำเร็จหรือไม่',
      'object PushSubscription ที่มี endpoint และ encryption key',
  ...
  {
    q: 'ทำไม subscribe options จึงต้องมี userVisibleOnly: true?',
    options: [
      'เพราะช่วยเพิ่ม performance',
      'เพราะ browser บังคับว่าทุก push message ต้องแสดง notification ที่ผู้ใช้เห็นได้',
      'เพราะเปิดใช้ background sync',
      'เพราะจำกัด subscription ไว้เฉพาะแท็บปัจจุบัน',
    ],
    answer: 1,
  },
  {
    q: 'ควรเก็บ VAPID private key ไว้ที่ไหน?',
    options: [
      'ใน manifest.webmanifest',
      'ในไฟล์ source ของ service worker',
      'เก็บไว้เฉพาะบน backend server เท่านั้น ไม่เปิดเผยต่อ browser',
      'ใน localStorage เพื่อให้คงอยู่',
    ],
    answer: 2,
  },
  {
    q: 'event ใดของ service worker ที่ทำงานเมื่อ server ส่ง push message?',
