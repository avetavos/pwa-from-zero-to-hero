...
| **เฉพาะ Chromium** | ณ ปี 2026 เฉพาะ browser ที่ใช้ Chromium เท่านั้นที่รองรับ PBS; Firefox และ Safari ไม่รองรับ |
...
แอปข่าวและแอปแผนที่ที่รีเฟรชเนื้อหาที่ cache ไว้ (หัวข้อข่าว, ข้อมูลจราจร) เป็นระยะเบื้องหลัง ให้พร้อมทันทีที่เปิดแอป คือ use case จริงทั่วไปของ API นี้ ไม่ว่าแอปนั้นจะใช้ Periodic Background Sync โดยตรงหรือกลไกอื่นก็ตาม

export const quizPeriodicSync = [
  {
    q: 'option minInterval ใน periodicSync.register() รับประกันอะไร?',
    options: [
      'sync ทำงานทุกๆ minInterval มิลลิวินาทีเป๊ะๆ',
      'sync ทำงานถี่ไม่เกิน minInterval — browser อาจเรียกน้อยกว่านั้นได้',
      'sync ทำงานอย่างน้อยหนึ่งครั้งต่อ minInterval',
      'ไม่รับประกันอะไร — browser ไม่สนใจค่านี้',
    ],
    answer: 1,
  },
  {
    q: 'ข้อใดจำเป็นสำหรับให้ Periodic Background Sync ทำงานได้?',
    options: [
      'push subscription ที่มี VAPID key',
      'PWA ต้องถูกติดตั้งและ browser ต้องใช้ Chromium',
      'ต้องได้รับ notification permission',
      'หน้าเว็บต้องเปิดอยู่ในแท็บ',
    ],
    answer: 1,
  },
  {
    q: 'event periodicsync ทำงานที่ไหน?',
    options: [
  ...
  {
    q: 'ทำไมจึงควรทำ fallback ที่ refresh ข้อมูลตอนโหลดหน้า?',
    options: [
      'เพราะ periodic sync ต้องขอ permission ทุกครั้งที่ทำงาน',
      'เพราะ browser อาจไม่เรียก periodicsync ถ้าเงื่อนไข engagement หรือ battery ไม่ผ่าน',
      'เพราะ periodicsync ทำงานแค่ครั้งเดียวต่อการติดตั้ง',
      'เพราะ cache ถูกล้างหลังจาก periodic sync ทุกครั้ง',
    ],
    answer: 1,
  },
];
