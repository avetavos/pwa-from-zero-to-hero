...
Vite PWA ecosystem ที่ Elk (Mastodon client) ใช้จริงมี vite-plugin-pwa generate manifest และ icon ชุดเต็มอัตโนมัติจาก config เดียว ([Elk's `package.json` ระบุ `vite-plugin-pwa`](https://github.com/elk-zone/elk)) และ VitePress sites หลายเจ้าก็ใช้ plugin เดียวกันนี้ทำให้ documentation site ติดตั้งเป็น PWA ได้โดยแทบไม่ต้องเขียน manifest เอง
