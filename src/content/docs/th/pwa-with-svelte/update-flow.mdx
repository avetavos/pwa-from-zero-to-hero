...
ด้วย `@vite-pwa/sveltekit` ใช้ API `virtual:pwa-register/svelte` — API นี้ให้ store `needRefresh` และ `offlineReady` รวมถึง `updateServiceWorker()` สำหรับ SW ในตัว ให้เชื่อมต่อ `updatefound` และ `statechange` เองและใช้ `postMessage` เพื่อให้ `skipWaiting()` ขึ้นอยู่กับการกระทำของผู้ใช้
</Callout>
...
Vite PWA ecosystem ฝั่ง SvelteKit ใช้ store `virtual:pwa-register/svelte` แจ้ง `needRefresh` ให้ UI แสดง toast ตามมาตรฐานเดียวกับฝั่ง React/Vue และ Elk แสดง prompt ให้ผู้ใช้กด reload เองเมื่อมีเวอร์ชันใหม่ แทนที่จะ force reload ระหว่างกำลังเลื่อนอ่าน timeline
