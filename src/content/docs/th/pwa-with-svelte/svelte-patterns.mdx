...
เก็บสถานะ PWA — สถานะ offline, ความพร้อมในการอัปเดต, และ install prompt — ใน Svelte store เพื่อให้ component ไหนก็ react ต่อ store นี้ได้ ป้องกัน browser-only API (`navigator`, `window`) ด้วย `typeof window !== 'undefined'` เสมอเพื่อป้องกัน SSR crash ใช้ `adapter-static` พร้อม `prerendered` asset สำหรับแอปที่ offline ได้ทั้งหมด
</Callout>
...
Vite PWA ecosystem ฝั่ง SvelteKit ใช้ store pattern ครอบ `virtual:pwa-register/svelte` ให้ UI reactive ต่อ update state ได้ทันที และเครื่องมือแบบ Squoosh ที่พอร์ตมาเป็น SvelteKit ก็จะใช้ Svelte store เก็บสถานะ installable/offline แบบเดียวกันให้ทุก component อ่านค่าเดียวกัน
