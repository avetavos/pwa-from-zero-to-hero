...
เพิ่ม `SvelteKitPWA()` ใน Vite plugins ของคุณ — plugin จะสร้าง service worker และ manifest สำหรับ SvelteKit build ใช้ `registerType: 'prompt'` ใน production เพื่อให้ผู้ใช้ควบคุมเวลาที่จะ apply การอัปเดต
</Callout>
...
`@vite-pwa/sveltekit` เป็นส่วนหนึ่งของ Vite PWA ecosystem เดียวกับที่ Elk (Mastodon client) พอร์ตแนวคิดเดียวกันมาใช้บน SvelteKit และเทมเพลต starter ของชุมชน SvelteKit หลายตัวก็ใช้ plugin นี้เป็นจุดเริ่มต้นมาตรฐานสำหรับทำ SvelteKit PWA
