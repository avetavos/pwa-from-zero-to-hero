...
Workbox ห่อหุ้ม strategy แบบ cache-first, network-first, และ stale-while-revalidate ไว้ คุณจึงเขียนโค้ด service worker ที่มีโอกาสผิดพลาดน้อยลง `vite-plugin-pwa` ไปไกลกว่านั้น — สร้างไฟล์ SW ทั้งไฟล์พร้อม precaching ให้ตอน build
</Callout>
...
Workbox สร้างและดูแลโดยทีม Chrome เอง ([source บน GitHub](https://github.com/GoogleChrome/workbox)) และ `vite-plugin-pwa` ก็ใช้ Workbox อยู่เบื้องหลังเพื่อสร้าง service worker และ precaching manifest ให้อัตโนมัติทุกครั้งที่ build ([vite-pwa-org.netlify.app](https://vite-pwa-org.netlify.app/))
