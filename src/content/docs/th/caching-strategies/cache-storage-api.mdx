...
clone Response ก่อนแคชเสมอ — body ของตัวเองคือ stream ที่ใช้ได้ครั้งเดียว ส่ง clone ให้ `cache.put()` แล้วคืน original กลับไปให้หน้าเพจ หรือสลับกันก็ได้ ไม่ว่าจะทางไหน คุณต้องมีออบเจ็กต์ Response ที่เป็นอิสระต่อกันสองตัว
</Callout>
...
Squoosh PWA บีบอัดรูปภาพแบบ open source ของ Google Chrome Labs cache asset ทุกตัวของแอปไว้ตั้งแต่ install ทำให้ใช้งานได้แบบ offline-first ตั้งแต่ครั้งแรกที่เปิด ([source บน GitHub](https://github.com/GoogleChromeLabs/squoosh)) — เป็น pattern precache-ทุกอย่างแบบเดียวกับที่บทเรียนนี้สร้างขึ้น
