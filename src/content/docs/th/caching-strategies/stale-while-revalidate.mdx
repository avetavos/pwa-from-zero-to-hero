...
HTTP `Cache-Control` directive `stale-while-revalidate` ([RFC 5861](https://developer.mozilla.org/en-US/docs/Web/HTTP/Caching); รองรับโดย CDN อย่าง Cloudflare และ Fastly) คือ pattern มาตรฐานที่ service worker เวอร์ชันในบทเรียนนี้ยืมชื่อและแนวคิดมา — ชั้น CDN ทำแบบนี้มาก่อน ก่อนที่ service worker จะนำแนวคิดเดียวกันมาใช้ฝั่ง client Workbox StaleWhileRevalidate strategy ก็ใช้แนวคิดเดียวกันนี้ และใช้กันแพร่หลายในเว็บข่าวและ e-commerce ที่ต้องการทั้งความเร็วของหน้า listing และข้อมูลที่ไม่เก่าเกินไป
