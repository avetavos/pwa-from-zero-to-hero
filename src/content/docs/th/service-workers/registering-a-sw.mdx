...
service worker ที่ `/sw.js` จะควบคุมทั้ง origin — scope เริ่มต้นของตัวเองคือ `/` วางไฟล์ไว้ที่ root ของเว็บไซต์เพื่อให้ครอบคลุมทั้งหมด การย้ายไฟล์เข้าไปในไดเรกทอรีย่อยจะจำกัด scope ให้แคบลงเหลือเพียงไดเรกทอรีย่อยนั้นแบบเงียบ ๆ
</Callout>
...
PWA checklist ของ web.dev เองแนะนำให้ register service worker หลัง `load` event เสมอ เพื่อไม่ให้แย่ง bandwidth กับ critical resource ตอนโหลดหน้าแรก case study ของ Twitter ก็อธิบาย rollout service worker แบบ deferred ในลักษณะเดียวกัน ([web.dev/twitter](https://web.dev/twitter/))
...
export const quizRegisterSw = [
  ...
  {
    q: 'คุณวาง sw.js ไว้ที่ /admin/sw.js scope เริ่มต้นของตัวเองคืออะไร?',
